...
void get_position(const nav_msgs::Odometry::ConstPtr& msg_odom){
...
int main( int argc, char** argv )
{
    ...
    ros::ServiceClient client = n.serviceClient<gaden_filament_simulator::GasPosition>("/odor_value");
	ros::Subscriber sub = n.subscribe<nav_msgs::Odometry>("/odom", 10, get_position);
    ...
    connector.id = 1;
	connector.scale.x = 10;
	connector.scale.y = 10;
    ...
    while (ros::ok())
    {
        ...
        if (know_sensor_pose)
        {
            ...
            gaden_filament_simulator::GasPosition srv;
            srv.request.x = x_pos;
            srv.request.y = y_pos;
            srv.request.z = z_pos;  
            ros::service::waitForService("/odor_value");          
            if (client.call(srv))
            {
                // if(srv.response.gas_conc[0] != 0){
                //     ROS_INFO("concentration: %f", srv.response.gas_conc[0]);
                // }
/*
...
float simulate_mox_as_line_loglog(gaden_filament_simulator::GasPositionResponse GT_gas_concentrations)
{
    if (first_reading)
    {
...
float simulate_pid(gaden_filament_simulator::GasPositionResponse GT_gas_concentrations)
{
    //Handle multiple gases
...
{	
    ...
    // private_nh.param<std::string>("simulation_data_0", simulator_data_filepath, " ");
