...
//==========================//
void CFilamentSimulator::CalParameters()
{
...
void CFilamentSimulator::initSimulator()
{
	...
	*/
	...
	num_moles_all_gases_in_cm3 = env_cell_numMoles/env_cell_vol;
	...
	//total number of moles in a filament
	filament_numMoles_of_gas = filament_moles_cm3_center*sqrt(8*pow(3.14159,3)) * pow(filament_initial_std,3.0);   
...
//                          //
//==========================//
void CFilamentSimulator::publish_markers(int gasType)
{
	//1. Clean old markers
...
void CFilamentSimulator::preMarker(){
	
	//Load wind snapshot (if necessary and availabe)
	if (sim_time-sim_time_last_wind >= windTime_step)
	{
		// Time to update wind!
		sim_time_last_wind = sim_time;
		if (allow_looping)
		{
			// Load wind-data
			read_wind_snapshot(current_wind_snapshot);
			// Update idx
			if (current_wind_snapshot >= loop_to_step){
				current_wind_snapshot = loop_from_step;
				wind_finished = true;
			}
			else
				current_wind_snapshot++;
		}
		else
			read_wind_snapshot(floor(sim_time/windTime_step));  //Alllways increasing
	}
	...
	publish_markers(gasType);

	
}

void CFilamentSimulator::postMarker(){
	//3. Update filament locations
	sim_time = sim_time + time_step;	//sec
	current_simulation_step++;
	if (current_simulation_step<numSteps) {
		alive = true;
	} else{
		alive = false;
		CalParameters();
